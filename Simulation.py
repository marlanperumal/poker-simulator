...
class HandProbability(object):
    def __init__(self, num_players=2, default_repeats=1000):
        self.num_players = num_players
        self.default_repeats = default_repeats

    def run(self, my_original_cards=None, community_cards=None, repeats=None, verbose=False):
        if repeats is None:
            repeats = self.default_repeats

        if my_original_cards is None:
            my_original_cards = []
        ...
        for _ in range(repeats):
            result = "win"
            deck = Deck()
            deck.extract(my_original_cards)
            deck.extract(community_cards)
            deck.shuffle()
            my_cards = my_original_cards + deck.deal(2 - len(my_original_cards))
        ...
        if verbose:
            ...
            print("With hand: ", my_original_cards)
...
if __name__ == "__main__":
    ...
    for i, rank_1 in enumerate(rank_list):
        ...
        for rank_2 in rank_list[:i]:
            suited_record = simulation.run(my_original_cards=[Card(rank_1, "Hearts"), Card(rank_2, "Hearts")])
            suited_probability.append(suited_record["win percentage"] + suited_record["draw percentage"])
        for rank_2 in rank_list[i:]:
            unsuited_record = simulation.run(my_original_cards=[Card(rank_1, "Hearts"), Card(rank_2, "Spades")])
